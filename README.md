...
Open  **func.py**  and replace the content of the file with the following code
 ...
 a.  Import the necessary Python modules, as shown in the following snippet
```Python
import io
import json
import logging
...
```
b. Define a function to parse the log data and invoke the SumoLogic API to ingest the logs.
```Python
...
For information about the format of the logs generated by the Oracle Cloud Infrastructure Logging service, see [Logging Format Overview.](https://docs.cloud.oracle.com/en-us/iaas/Content/Logging/Reference/top_level_logging_format.htm#top_level_logging_format)    
 ...
 c. Retrieve the log entrie(s) via Service Connector Hub as part of the Function call payload and loop through the logs one by one.
```Python
    try:
